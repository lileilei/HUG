...
(function (S, underfine) {
    ...
    var moduleCache = {},//模块方法依赖关系缓存
        useModule={},
        Notloadmodule={},//加载剩余模块
        allmodule ={};//所有模块

    var createScript =function(filepath,onload){
        var debug = S.Config.DEBUG || false;
        if (debug) {
    ...
    function combineReqs(key,arrs){
        if(arrs.length>0){
            ...
            temp='moduleCache["'+key+'"].fn(S,'+temp+')';
            return temp;
        }else{
    ...
    var useRequires =function(reqs,func) {
        ...
        _.each(reqs,function(mod){
            ...
            if(debug){
                ...
                eval('useModule["'+mod+'"].fn(S,'+combineReqs(mod,moduleCache[mod].reqs)+')');
            }
...
HUG.add('button/buttonMenu',function(S,button){
...
HUG.add('color/color',function(S){
    return {
        red:"红色",
        yellow:"黄色"
    }
